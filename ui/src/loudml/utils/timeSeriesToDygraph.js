...
const timeSeriesTransform = (raw = []) => {
  ...
  const sortedLabels = labels // _.sortBy(labels, 'label')
  const tsMemo = {}
...
export const customBars = series => {
  const {labels, timeSeries, dygraphSeries} = series

  if (labels.length-1 < 3) {
    return series
  }

  const dygraph = {}
  dygraph[labels[2]] = dygraphSeries[labels[2]] // mid value
  ...
  return {
    ...
    timeSeries: timeSeries.map(t => [
      t[0],           // time
      [...t.slice(1)] // lower, mid, upper
    ]),
    dygraphSeries: dygraph,
  }
