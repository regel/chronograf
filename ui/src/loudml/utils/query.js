...
const createPrefixedQueryFields = (prefix, model, database) => {
    const {name, features, bucket_interval} = model
    const fields = features.map(f => `LAST("${prefix}${f.name}")`).join(' + ')
    return `SELECT ${fields} AS "${prefix}${name}" FROM "${database}"."autogen"."prediction_${name}" WHERE time > :dashboardTime: AND time < :upperDashboardTime: GROUP BY time(${bucket_interval}) FILL(null)`
}
...
const createQueryFields = (model, database) => {
    ...
    // SELECT ${metric}("${field-name}") AS "${metric}-${field-name}" FROM "${database}"."autogen"."${measurement}" WHERE time > :dashboardTime: AND time < :upperDashboardTime: GROUP BY time(${bucket_interval}) FILL(null)
}
